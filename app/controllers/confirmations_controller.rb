class ConfirmationsController < Milia::ConfirmationsController
  ...
  def show
    if @confirmable.new_record?  ||
      ...
      # Next two lines added as mods to original Milia show method
      self.resource = resource_class.confirm_by_token(params[:confirmation_token])
      yield resource if block_given?
      
      # Deleted the following line from original Milia show method
      # super #this will redirect
      
      # Add "if" block below to original Milia code
      if resource.errors.empty?
        set_flash_message(:notice, :confirmed) if is_flashing_format?
      end
      
      if @confirmable.skip_confirm_change_password
        #original Melia code was follows:
        # sign_in_tenanted(resource)
        sign_in_tenanted_and_redirect(resource)
      end
    else
      log_action( "password set form" )
      # Added flash notice below to origina Milia code
  ...
  # Add below method to original Milia code
  ...
  def set_confirmable()
    # Made change to the original Milia @confirmable assignment statement that was as below:
    # @confirmable = User.find_or_initialize_with_error_by(:confirmation_token, confirmation_token)
    
    # @confirmable assignment done made from params rather than confirmation_token as done in line above
